...
public class VMGeneral extends MyViewModel<Boolean> {
    ...
    public void categoriesOfProducts(int id) {
        new UCGeneral().categoriesOfProducts(id, this.getGenericClosure(categories));
    }
    public void categoriesOfPosts(int id) {
        new UCGeneral().categoriesOfPosts(id, this.getGenericClosure(categories));
    }
    public void cities() {
    ...
    public void follow(Category model){
        new UCFollowing().follow(new FollowRequest(model.getId(), !model.getFollowed(), "cat"), this.getGenericClosure(follow));
    }
    public void follow(City model){
        new UCFollowing().follow(new FollowRequest(model.getId(), !model.getFollowed(), "city"), this.getGenericClosure(follow));
    }
